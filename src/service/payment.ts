...
import { data } from '../../dist/controller/admin/merchant.controller';
...
export const createTransaction = async (
  ...
  const mName = request.merchantName.toLowerCase() || forced ;
  ...
  if (mName === "hilogate") {
    ...
    const hilClient = new HilogateClient(hilCfg);
      ...
      const apiResp = await hilClient.createTransaction(payload);
  ...
  if (mName === "piro" || mName === "genesis") {
    ...
    const baseCallback =
      genesisCfg?.callbackUrl || piroCfg?.callbackUrl || config.api.callbackUrl;
  ...
  if (mName === "ifp") {
    ...
    const paymentChannel =
      request.paymentChannel || ifpCfg.paymentChannel || "qris";
  ...
  if (mName === "gidi") {
    ...
    if (!rawCfg.credentialKey)
      throw new Error("Gidi credential missing credentialKey");
    ...
    try {
      ...
      throw new Error(
        `Gidi QRIS generation failed: ${err.message || "unknown"}`
      );
    }
  ...
  if (mName === "gv" || mName === "gudangvoucher") {
    ...
    const { merchantId, merchantKey, qrisUrl } = config.api
      .gudangvoucher as any;
...
export async function processHilogatePayload(payload: {
  ...
  const newSetSt =
    settlement_status?.toUpperCase() ?? (isSuccess ? "PENDING" : null);
...
export const transactionCallback = async (request: Request) => {
  ...
  if (sigHeader) {
    ...
    const exp = crypto
      .createHmac("sha256", config.api.tcpp.secretKey)
      .update(raw)
      .digest("hex");
    if (sigHeader !== exp) throw new Error("Invalid 2C2P signature");
    const refId2 =
      body.originalPartnerReferenceNo || body.invoiceNo || body.referenceNo;
    if (!refId2) throw new Error("Missing 2C2P referenceId");
    const exists2 = await prisma.transaction_callback.findFirst({
      where: { referenceId: refId2 },
    });
    if (!exists2) {
      await prisma.transaction_callback.create({
        data: { referenceId: refId2, requestBody: body },
      });
        ...
        try {
          ...
          if (cfg) {
            ...
            await processIng1Update({
              ...
              paymentReceivedTime:
                parseIng1Date(
                  data.paid_at ?? data.payment_received_time ?? data.paidAt
                ) ?? undefined,
              settlementTime:
                parseIng1Date(
                  data.settlement_time ?? data.settled_at ?? data.settlementTime
                ) ?? undefined,
              expirationTime:
                parseIng1Date(
                  data.expired_at ?? data.expiration_time ?? data.expirationTime
                ) ?? undefined,
            });
          ...
          logger.error(
            `[checkPaymentStatus] ING1 requery failed for ${order.id}: ${err.message}`
          );
        }
      } else {
