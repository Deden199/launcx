...
export async function getClientDashboard(req: ClientAuthRequest, res: Response) {
  try {
    ...
    // (10) Parallel queries - USE aggregateRaw for safer settlementTime handling
    const [metricsRaw, orders, totalRows] = await Promise.all([
      // ✅ FIX: Use aggregateRaw to safely handle settlementTime
      ...
      // ✅ Select tanpa settlementTime untuk hindari decode error
    ...
    // (11) Parse aggregateRaw results - ✅ FIXED TYPE CONVERSION
    const metricsGrouped = Array.isArray(metricsRaw) 
      ? (metricsRaw as any[]).map((m: any) => ({
          status: m._id,
          _sum: {
            amount: m.totalAmount ?? 0,
            settlementAmount: m.totalSettlement ?? 0,
            pendingAmount: m.totalPending ?? 0,
          },
        }))
      : []
    ...
    const metricsWithExpired = metricsGrouped.map((g) => {
      ...
      const isExpired =
        g.status === ORDER_STATUS.PENDING &&
        orders.some((o) => o.trxExpirationTime && new Date(o.trxExpirationTime) < now)
