...
export default function WithdrawPage() {
  ...
  const [bulkInfo, setBulkInfo] = useState({ ok: 0, fail: 0, queued: 0 });
  const [bulkError, setBulkError] = useState<string>('');

  // ===== OTP (Bulk) =====
  const [bulkOtp, setBulkOtp] = useState('');
  const [bulkOtpErr, setBulkOtpErr] = useState<string>('');
  const [otpCooldown, setOtpCooldown] = useState<number>(0); // detik
  const [otpInfoMsg, setOtpInfoMsg] = useState<string>('');   // info friendly
  ...
  useEffect(() => {
    const hasErr = bulkRows.some(r => (r.errors?.length ?? 0) > 0);
    if (!bulkRows.length || hasErr) {
      setBulkOtp('');
      setBulkOtpErr('');
      setOtpInfoMsg('');
      setOtpCooldown(0);
  ...
  const resolveProviderBank = useCallback(
    (provider: Provider, bankCode: string) => {
      ...
      const piroMeta = provider === 'piro' ? resolvePiroBankMeta(bankObj?.name, bankCode) : null;
  ...
  function validateHeaders(rawHeaders: any[]): { ok: boolean; reason?: string } {
    ...
    const missing = (REQUIRED_HEADERS as readonly string[]).filter((k) => !headers.includes(k));
    if (missing.length) {
  ...
  const loadSubMerchantsFromAPI = useCallback(async () => {
    try {
      setLoading(true);
      try {
        const response = await apiClient.get<SubMerchantResponse[]>('/client/withdrawals/submerchants', {
          params: { clientId: 'all' },
        });
        const subMerchantsFromAPI = response.data;
        if (subMerchantsFromAPI.length > 0) {
          const transformedData: SubMerchant[] = subMerchantsFromAPI.map((item) => ({
            id: item.id,
            name: item.name,
      ...
      if (subMerchantsFromJWT.length > 0) {
        const transformedData = subMerchantsFromJWT.map((item: SubMerchantResponse) => ({
  ...
  useEffect(() => {
    ...
    apiClient
      .get<{ banks: { code: string; name: string }[] }>('/banks', { params: { provider: apiBanksParam }, signal: ctl.signal })
  ...
  const validateAccount = async () => {
    ...
    try {
      ...
      const res = await apiClient.post('/client/withdrawals/validate-account', body, {
        validateStatus: () => true,
        timeout: 20000,
      });
      ...
      if (res.status === 200) {
        ...
        const apiErr = res.data?.error || `Validasi gagal (status ${res.status})`;
        setIsValid(false);
        setError(apiErr);
      }
    } catch {
  ...
  // =============================================
  // BULK WITHDRAWAL (dengan retry-OTP via Submit)
  // =============================================
  ...
  const requestBulkOtp = async (subIdForOtp: string) => {
    try {
      const payload = { merchantId: MERCHANT_ID, subMerchantId: subIdForOtp, reason: 'withdrawal_bulk' };
      const resReq = await apiClient.post('/client/withdrawals/otp/request', payload, { validateStatus: () => true });
      if (resReq.status === 200 || resReq.status === 201) {
        setBulkError('📨 OTP sudah dikirim. Masukkan 6 digit OTP lalu klik Submit untuk memproses.');
        setOtpInfoMsg('OTP terkirim. Cek kanal terdaftar.');
        setOtpCooldown(60);
      } else {
        setBulkError(resReq.data?.error || resReq.data?.message || 'Gagal mengirim OTP.');
      }
    } catch (e: any) {
      setBulkError(e?.message || 'Network error saat kirim OTP.');
    }
  };
      ...
      <div className="mx-auto max-w-7xl p-4 sm:p-6">
        ...
        {pageError && (
          <div className="mb-4 rounded-xl border border-rose-900/40 bg-rose-950/40 p-3 text-rose-300">
            ...
            <button onClick={refetchAll} className="rounded-lg border border-rose-800 bg-rose-900/40 px-3 py-1.5 text-sm hover:bg-rose-900/60">
              🔁 Try Again
            </button>
          </div>
        )}
        ...
        <div className="mb-6 grid grid-cols-1 gap-3 md:grid-cols-3">
          <div className="md:col-span-2 rounded-2xl border border-neutral-800 bg-neutral-900/60 p-3">
            ...
            {loading ? (
              <div className="text-sm text-neutral-500">Loading sub-wallets...</div>
            ) : (
                  ...
                  subs.map((s) => (
                    <button
                      ...
                      className={`rounded-xl border px-3 py-2 text-left transition ${
                        s.id === selectedSub ? 'border-sky-500 bg-sky-500/10' : 'border-neutral-800 hover:bg-neutral-800/60'
                      }`}
                    >
                      <div className="text-sm font-medium">{s.name || `Sub ${s.id.slice(0, 6)}`}</div>
                      <div className="text-xs text-neutral-400">{money(s.balance)}</div>
                    </button>
                  ))
                ) : (
                  <div className="text-sm text-neutral-500">Tidak ada sub-wallet yang tersedia.</div>
                )}
              </div>
            )}
          </div>
        ...
        <div className="mb-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setOpen(true)} className="inline-flex items-center gap-2 rounded-xl border border-neutral-800 bg-neutral-900 px-3 py-2 text-sm hover:bg-neutral-800/60">
        ...
        <section className="rounded-2xl border border-neutral-800 bg-neutral-900/70 p-4 sm:p-5">
          ...
          <div className="mb-4 grid grid-cols-1 gap-3 md:grid-cols-4">
            ...
            <div className="relative md:col-span-2">
              <DatePicker
                ...
                dayClassName={() => 'rounded-md !text-neutral-100 hover:!bg-neutral-800 focus:!bg-neutral-800'}
                withPortal
                ...
                renderCustomHeader={({ date, decreaseMonth, increaseMonth, prevMonthButtonDisabled, nextMonthButtonDisabled }) => (
                  <div className="flex items-center justify-between border-b border-neutral-800 bg-neutral-900 px-2.5 py-2">
                    <button type="button" onClick={decreaseMonth} disabled={prevMonthButtonDisabled} className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-neutral-800 text-neutral-300 hover:bg-neutral-800 disabled:opacity-40">
                      ‹
                    </button>
                    <div className="text-sm font-semibold">
                      {date.toLocaleString('en-US', { month: 'long', year: 'numeric' })}
                    </div>
                    <button type="button" onClick={increaseMonth} disabled={nextMonthButtonDisabled} className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-neutral-800 text-neutral-300 hover:bg-neutral-800 disabled:opacity-40">
                      ›
                    </button>
                  </div>
                )}
              ...
              <table className="min-w-[1000px] w-full text-sm">
                <thead className="sticky top-0 z-10">
                  <tr className="border-b border-neutral-800 bg-neutral-900/80 backdrop-blur">
                    {['Created At','Paid At','Ref ID','Type','Bank','Account','Account Name','Bulk ID','Wallet','Amount','Fee','Net Amount','Status'].map((h) => (
                ...
                <tbody>
                  {withdrawals.length ? (
                    ...
                    <tr>
                      <td colSpan={13} className="px-3 py-10 text-center text-neutral-400">
                        No data
                      </td>
                    </tr>
                  )}
          ...
          <div className="mt-4 flex flex-col items-center justify-between gap-3 sm:flex-row">
            ...
            <div className="flex items-center gap-2 text-sm">
              <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="inline-flex h-9 items-center rounded-lg border border-neutral-800 px-2.5 disabled:opacity-50 hover:bg-neutral-800/60">
                ‹
              </button>
              <span className="min-w-[70px] text-center">
                {page}/{totalPages}
              </span>
              <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="inline-flex h-9 items-center rounded-lg border border-neutral-800 px-2.5 disabled:opacity-50 hover:bg-neutral-800/60">
                ›
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-4xl rounded-2xl bg-neutral-900 p-6 shadow-xl" role="dialog" aria-modal="true">
            <div className="mb-4 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button type="button" onClick={downloadBulkTemplate} className="rounded-lg border border-neutral-700 px-3 py-1.5 text-sm hover:bg-neutral-800/60">
                  Download Template
                </button>
                <button
                  type="button"
            ...
            {(bulkError || otpInfoMsg) && (
              ...
              >
                {bulkError || otpInfoMsg}
                {otpCooldown > 0 && (
                  <span className="ml-2 opacity-80"> | Kirim ulang tersedia dalam {otpCooldown}s</span>
                )}
              </div>
            )}
            ...
            <div className="mb-4 rounded-xl border border-neutral-800 bg-neutral-950 p-3">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <label className="text-sm text-neutral-300">Unggah file <b>.csv</b> sesuai template</label>
                <input
                  key={filePickerKey}
                  ref={fileInputRef}
                  type="file"
            ...
            {/* OTP Input (muncul kalau data siap dan tak ada error parsing) */}
            {bulkRows.length > 0 && !bulkRows.some(r => (r.errors?.length ?? 0) > 0) && (
              <div className="mb-4 rounded-xl border border-neutral-800 bg-neutral-950 p-3">
                <label className="mb-1 block text-sm text-neutral-300">
                  OTP <span className="text-neutral-500">(6 digit)</span>
                </label>
                ...
                <p id="bulk-otp-help" className="mt-1 text-xs text-neutral-400">
                  Belum terima OTP? Klik <b>Submit</b> untuk mengirim atau mengirim ulang.
                  {otpCooldown > 0 && <> Kirim ulang tersedia dalam <b>{otpCooldown}s</b>.</>}
                </p>
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-neutral-900/80 backdrop-blur">
                  <tr className="border-b border-neutral-800">
                    {['#','Sub-wallet','Type','BankCode','AccountNumber','Bulk ID','Amount','Status','Errors'].map((h) => (
                ...
                <tbody>
                  {bulkParsing ? (
                    <tr>
                      <td colSpan={9} className="px-3 py-8 text-center text-neutral-400 animate-pulse">Sedang memuat data...</td>
                    </tr>
                  ) : bulkRows.length === 0 ? (
                    <tr>
                      <td colSpan={9} className="px-3 py-8 text-center text-neutral-500">Belum ada data. Download template lalu unggah file.</td>
                    </tr>
                  ) : (
                    bulkRows.map((r) => (
                      <tr key={r.idx} className="border-b border-neutral-800">
                        <td className="px-3 py-2">{r.idx}</td>
                        <td className="px-3 py-2">{subs.find((s) => s.id === r.subMerchantId)?.name || r.subMerchantId}</td>
            ...
            <div className="flex items-center justify-between">
              ...
              >
                {bulkSubmitting ? 'Processing…' : BULK_DUMMY_MODE ? 'Simulate Bulk' : 'Submit'}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="w-full max-w-lg rounded-2xl border border-neutral-800 bg-neutral-900 p-5" onClick={(e) => e.stopPropagation()}>
            ...
            <form className="grid gap-3" onSubmit={submit}>
              ...
              <div>
                ...
                <input name="accountNumber" value={form.accountNumber} onChange={handleChange} className="h-10 w-full rounded-lg border border-neutral-800 bg-neutral-900 px-3 text-sm outline-none" required />
              </div>

              <div>
                <label className="mb-1 block text-sm text-neutral-300">Account Name</label>
                <div className="relative">
                  <input readOnly value={form.accountName} placeholder="Isi otomatis setelah validasi" className="h-10 w-full rounded-lg border border-neutral-800 bg-neutral-900 px-3 text-sm outline-none" />
              ...
              <div>
                ...
                <input type="number" name="amount" value={form.amount} onChange={handleChange} className="h-10 w-full rounded-lg border border-neutral-800 bg-neutral-900 px-3 text-sm outline-none" required />
              </div>

              <div>
                <label className="mb-1 block text-sm text-neutral-300">OTP</label>
                <input name="otp" value={form.otp} onChange={handleChange} className="h-10 w-full rounded-lg border border-neutral-800 bg-neutral-900 px-3 text-sm outline-none" required />
              </div>
              ...
              <div className="mt-2 flex items-center justify-between gap-2">
                <button
                  ...
                  disabled={busy.validating || !form.bankCode || !form.accountNumber || !selectedSub}
